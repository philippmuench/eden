...
amino_acids_nogap = ['A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I', 'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V']
...
def unweighted_freq_count(col, seq_weights, pc_amount):
    """ Return the weighted frequency count for a column--with pseudocount."""
    # by pmuench
    # equal seq weights
    seq_weights = [1.] * len(col)
    ...
    aa_num = 0
    freq_counts = len(amino_acids_nogap)*[pc_amount] # in order defined by amino_acids

    for aa in amino_acids_nogap:
        for j in range(len(col)):
            if col[j] == aa:
                freq_counts[aa_num] += 1 * seq_weights[j]
...
def shannon_entropy(col, sim_matrix, bg_distr, seq_weights, gap_penalty=1):
    ...
    fc = unweighted_freq_count(col, seq_weights, 1)
  #  print(fc)
    gap_sign = 20 # this is the "-" sign  
    max = 0
    index = 0
    index_max = 0
    ...
    for aa in fc:
        ...
        if max == 1:
        	 index_max = gap_sign
    sys.stdout.write(amino_acids[index_max])


   # h = 0.
   # for i in range(len(fc)):q
#	if fc[i] != 0:
